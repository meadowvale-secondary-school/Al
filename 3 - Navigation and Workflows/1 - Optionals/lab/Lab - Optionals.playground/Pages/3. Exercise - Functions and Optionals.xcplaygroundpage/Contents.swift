...
func checkAge(_ enteredAge:String) -> Int?{
    if let age = Int(enteredAge){
        if age < 18{
            print("Welcome!")
        } else {
            print("Sorry, you're too young to enter the club")
        }
    } else {
        print("You messed up entering your age. It's literally 2 numbers... Try again")
    }
   return Int(enteredAge)
}
...
func order(_ name: String) -> Double? {
    var cost: Double? = nil
    if stock[name] != 0{
        cost = prices[name]
    } else {
